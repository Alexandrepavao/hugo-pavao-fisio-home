...
const Cohorts = ({ course }: { course: Course }) => {
  ...
  const create = async (e: FormEvent) => { e.preventDefault(); if (!name.trim()) return m.err("Informe o nome da turma."); const { error } = await supabase.from("cohorts").insert({ org_id: course.org_id, course_id: course.id, name: name.trim(), starts_on: starts || null, ends_on: ends || null }); error ? m.err(errText(error)) : (m.ok("Turma criada."), setName(""), void qc.invalidateQueries({ queryKey: ["cohorts", course.id] })); };
  const add = async (pid: string) => { const { error } = await supabase.rpc("cohort_add_member", { p_cohort: sel, p_person: pid }); error ? m.err(errText(error)) : (m.ok("Aluno adicionado: acesso por turma concedido até o fim da turma."), setSearch(""), void qc.invalidateQueries({ queryKey: ["cohort-members", sel] }), void qc.invalidateQueries({ queryKey: ["ents", course.id] })); };
  return (<><Msg m={msg} />
