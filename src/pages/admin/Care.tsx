...
const Patients = () => {
  ...
  const assign = async (e: FormEvent) => { e.preventDefault(); if (!sel || !content) return m.err("Selecione o conteúdo."); const { error } = await supabase.rpc("care_assign", { p_person: sel, p_content: content, p_phase: phase, p_valid_until: null, p_note: note || null }); if (error) m.err(errText(error)); else { m.ok("Conteúdo liberado ao paciente."); setNote(""); refresh(); } };
  ...
  const send = async (e: FormEvent) => { e.preventDefault(); if (!sel || !reply.trim()) return; const { error } = await supabase.rpc("care_reply", { p_person: sel, p_body: reply }); if (error) m.err(errText(error)); else { setReply(""); refresh(); } };
  return (<><Msg m={msg} /><State loading={rels.isLoading} error={rels.error} empty={rels.data?.length === 0} emptyText="Você ainda não tem pacientes vinculados. O vínculo é criado pelo gestor da unidade." />
    {rels.data && rels.data.length > 0 && <div className="grid gap-6 lg:grid-cols-[16rem_1fr]">
      ...
      {sel && detail.data && <div className="space-y-8">
        ...
        <section><h3 className="text-lg mb-2">Conteúdos liberados</h3><ul className="space-y-1">{detail.data.a.map((a) => <li key={a.id} className="flex justify-between text-sm hp-card p-2"><span>{a.content.title} · {fmtDateTime(a.released_at)} {a.revoked_at && "· revogado"}</span>{!a.revoked_at && <button className="text-destructive" onClick={() => revoke(a.id)}>Revogar</button>}</li>)}</ul></section>
        ...
        <section><h3 className="text-lg mb-2">Canal de dúvidas</h3><ul className="space-y-2 mb-3">{detail.data.msgs.map((x) => <li key={x.id} className={`p-2 border border-border max-w-lg ${x.sender === "professional" ? "bg-muted ml-auto" : "bg-card"}`}><span className="text-xs text-muted-foreground">{x.sender === "professional" ? "Você" : "Paciente"} · {fmtDateTime(x.created_at)}</span><br />{x.body}</li>)}</ul>
          <form onSubmit={send} className="flex gap-2"><label className="sr-only" htmlFor="rp">Resposta</label><textarea id="rp" rows={2}   value={reply} onChange={(e) => setReply(e.target.value)} /><button className={btnGhost}>Responder</button></form></section>
      </div>}</div>}</>);
};
...
const Contents = () => {
  ...
  return (<><Msg m={msg} /><form onSubmit={add} className="hp-card p-4 mb-4 grid gap-3 sm:grid-cols-2" noValidate>
    <div><label htmlFor="ct" className="block text-xs mb-1">Título</label><input id="ct"   value={title} onChange={(e) => setTitle(e.target.value)} /></div>
    ...
    <div><label htmlFor="cu" className="block text-xs mb-1">Unidade</label><select id="cu"   value={unit} onChange={(e) => setUnit(e.target.value)}><option value="">…</option>{units.data?.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>
    <div><label htmlFor="cf" className="block text-xs mb-1">Vídeo/arquivo (bucket privado)</label><input id="cf" type="file"   onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></div>
    <div className="sm:col-span-2"><label htmlFor="cb" className="block text-xs mb-1">Texto da orientação</label><textarea id="cb" rows={4}   value={body} onChange={(e) => setBody(e.target.value)} /></div><button className={btnGhost + " w-fit"}>Criar conteúdo</button></form>
...
const Links = () => {
  ...
  const link = async (e: FormEvent) => { e.preventDefault(); if (!person?.unit_id || !prof) return m.err("Selecione paciente (com unidade) e profissional."); const { error } = await supabase.rpc("care_link", { p_person: person.id, p_professional: prof, p_unit: person.unit_id }); if (error) m.err(errText(error)); else { m.ok("Vínculo criado."); setPerson(null); setSearch(""); void qc.invalidateQueries({ queryKey: ["all-rels"] }); } };
  const unlink = async (id: string) => { if (!(await confirmDialog("Revogar vínculo assistencial?", "O profissional perde o acesso ao paciente e os conteúdos liberados por ele são revogados.", "Revogar", true))) return; const { error } = await supabase.rpc("care_unlink", { p_id: id }); if (error) m.err(errText(error)); else { m.ok("Vínculo revogado."); void qc.invalidateQueries({ queryKey: ["all-rels"] }); } };
  return (<><Msg m={msg} /><form onSubmit={link} className="hp-card p-4 mb-4 grid gap-3 sm:grid-cols-3 items-end">
    <div><label htmlFor="lp" className="block text-xs mb-1">Paciente</label><input id="lp"   value={person ? person.full_name : search} onChange={(e) => { setPerson(null); setSearch(e.target.value); }} />{found.data?.map((p) => <button type="button" key={p.id} className="block w-full text-left p-2 border border-border hover:bg-muted" onClick={() => setPerson(p)}>{p.full_name}</button>)}</div>
    <div><label htmlFor="lf" className="block text-xs mb-1">Fisioterapeuta</label><select id="lf"   value={prof} onChange={(e) => setProf(e.target.value)}><option value="">…</option>{users.data?.map((u) => <option key={u.user_id} value={u.user_id}>{u.name}</option>)}</select></div><button className={btnGhost + " sm:w-fit"}>Vincular</button></form>
    <p className="text-sm text-muted-foreground mb-3">Este vínculo autoriza o profissional a acompanhar o paciente. Gestores não ganham acesso clínico por criar vínculos.</p>
    <State loading={rels.isLoading} error={rels.error} empty={rels.data?.length === 0} emptyText="Nenhum vínculo ativo." />
    {rels.data && rels.data.length > 0 && <Table head={["Paciente", "Fisioterapeuta", "Unidade", "Desde", ""]}>{rels.data.map((r) => <tr key={r.id}><Td>{r.patient_name}</Td><Td>{r.professional_name}</Td><Td>{r.unit_name}</Td><Td>{fmtDateTime(r.created_at)}</Td><Td><button className={btnDanger + " hp-btn-sm"} onClick={() => unlink(r.id)}>Revogar</button></Td></tr>)}</Table>}</>);
};
