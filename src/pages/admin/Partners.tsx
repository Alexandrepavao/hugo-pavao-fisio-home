...
const Partners = () => {
  ...
  return (<div>
    ...
    {tab === "repasses" && (<><form onSubmit={addPayout} className="bg-card border border-border p-4 mb-4 grid gap-3 sm:grid-cols-5 items-end" noValidate>
      <div><label htmlFor="rpp" className="block text-xs mb-1">Parceiro</label><select id="rpp" className={inputCls} value={pp} onChange={(e) => setPp(e.target.value)}><option value="">…</option>{partners.data?.map((p) => <option key={p.person_id} value={p.person_id}>{p.person.full_name}</option>)}</select></div>
      <div><label htmlFor="rpu" className="block text-xs mb-1">Unidade</label><select id="rpu" className={inputCls} value={unit} onChange={(e) => setUnit(e.target.value)}><option value="">…</option>{units.data?.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>
      <div className="sm:col-span-2"><label htmlFor="rpd" className="block text-xs mb-1">Descrição</label><input id="rpd" className={inputCls} value={desc} onChange={(e) => setDesc(e.target.value)} /></div>
      <div><label htmlFor="rpv" className="block text-xs mb-1">Valor (R$)</label><input id="rpv" className={inputCls} value={amount} onChange={(e) => setAmount(e.target.value)} /></div><button className="btn-primary !py-2 sm:w-fit">Criar</button></form>
      ...
      {payouts.data && payouts.data.length > 0 && <Table head={["Parceiro", "Descrição", "Valor", "Estado", ""]} right={[2]}>{payouts.data.map((p) => <tr key={p.id}><Td>{p.partner.full_name}</Td><Td>{p.description}</Td><Td num>{brl(p.amount_cents)}</Td><Td>{{ pending: "Pendente", authorized: "Autorizado", paid: "Pago", cancelled: "Cancelado" }[p.status]}</Td>
        <Td>{p.status === "pending" && <button className={btnGhost + " !py-1"} onClick={() => setSt(p.id, "authorized")}>Autorizar</button>}{p.status === "authorized" && <button className={btnGhost + " !py-1"} onClick={() => setSt(p.id, "paid")}>Marcar pago</button>}</Td></tr>)}</Table>}</>)}
  </div>);
};
