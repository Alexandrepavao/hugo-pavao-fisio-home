...
- Publicado hoje (2026-09-22) a partir do commit `52cac0e` (branch `feature/hp-group-hub`), deploy id `6ab2e26a4540b2bc161ef712`. Build "ready", 1 Function (`send-email`) e 1 Edge Function (`social-meta`) publicadas.
- Env vars `VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY` configuradas nos contextos `branch-deploy` e `deploy-preview` deste site, apontando **exclusivamente** ao Supabase Dev (`fsvtzowcwhvwtluwrhnb`). Nenhuma chave privilegiada. **Achado corrigido nesta sessão**: essas variáveis estavam ausentes em todos os contextos (site com zero env vars configuradas), o que fazia qualquer build mostrar "Backend não configurado" — root cause confirmado antes da correção.
...
Variáveis `RESEND_API_KEY`/`EMAIL_FROM` (function-scope, para `/api/send-email`); URLs de redirecionamento no Supabase Auth; resolver o site Netlify não documentado (`leafy-cascaron-325147`, ver acima). SEO/OG por página **já está implementado** via Edge Function (`netlify/edge-functions/social-meta.ts`, publicada neste deploy) — pendente apenas validação por crawler externo real, bloqueada pela proteção de equipe (ver `docs/release-readiness.md`).
