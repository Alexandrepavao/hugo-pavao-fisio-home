...
## Pesquisas (`research_*` — migration 032, sessão 2026-09-22, item A)
Sistema geral de pesquisas com perguntas próprias — distinto do `public.surveys` (NPS fixo pós-atendimento, já existente). Público-alvo por `person_kinds` (patient/partner/student). **Identificada ou anônima de verdade**: anônima nunca grava `person_id` na resposta (ausência estrutural, não ocultação em relatório) — como consequência honesta, não impede reenvio da mesma pessoa (não há identificador pra checar sem quebrar o anonimato). **Versionamento imutável**: `research_questions` é a cópia de trabalho editável; publicar — e qualquer edição de pergunta feita DEPOIS de publicada — congela um snapshot em `research_survey_versions`; cada resposta grava a versão vigente no envio, então editar a pesquisa depois nunca muda o significado de respostas já registradas. **k-anonimato no dashboard**: quebra por pergunta/opção só com 5+ respostas no recorte de período/unidade (mesmo piso de `corporate_indicators`) — participação total aparece sempre, a distribuição de respostas (potencialmente sensível, inclusive de saúde) só com base suficiente. Lista de respostas individuais é tela separada do dashboard agregado, e só existe pra pesquisa identificada.

## Contas corporativas (`corporate_*` — migrations 009/033/034, sessão 2026-09-22, item B)
Estende o schema já existente (`corporate_accounts`/`corporate_members`/`corporate_indicators`) em vez de recriar. Empresa pagadora é sempre um **cliente do HP** dentro da mesma organização (nunca uma organização nova); pessoa atendida é sempre um vínculo (`corporate_members`) a um cadastro já existente — nunca duplica. `corporate_account_report` (novo) dá utilização (atendimentos) e valores (recebido) por período, com o mesmo piso de k-anonimato de 5 vínculos.

## Retentativa de eventos (migration 035, sessão 2026-09-22, item C)
Estende `domain_events`/`event_handlers`/`automation_runs`/`dispatch_event` (já existentes) com a camada de fila: reserva por `FOR UPDATE SKIP LOCKED` (nunca dois workers no mesmo evento), reserva órfã (>5min sem conclusão) volta pra fila automaticamente, backoff progressivo (2^tentativas minutos, teto 60), limite de 8 tentativas, distinção falha temporária (retentativa) x definitiva (`errcode 'P0002'`, vai direto pra `dead`). `pg_cron` processa a fila a cada minuto de verdade (job `domain-events-retry`) — não é simulação. Reprocessamento manual (`retry_failed_events()`) passou a reservar também e fica auditado.

## Metadados sociais (Netlify Edge Function `social-meta.ts`, sessão 2026-09-22, item D)
Injeta título/descrição/canonical/Open Graph/twitter:card/imagem (sempre absoluta) no HTML cru de páginas do HP Pages **publicadas**, antes de qualquer JS rodar. Rascunho/desativada/inexistente nunca expõe conteúdo — ganham `noindex, nofollow`. Todo texto do editor passa por escape de HTML.
...
- `corporate_account_report`: filtrava atendimentos por `appointments.starts_at`, coluna que não existe (`appointments.period` é um `tstzrange`) — corrigido para `lower(ap.period)`, mesmo padrão já usado em `dashboard_metrics`/`my_appointments` (migration 034).
- **Netlify Edge Function de metadados sociais**: as tags `og:*`/`twitter:*` padrão do `index.html` não eram removidas antes de injetar as da página — ficavam duplicadas, e a maioria dos crawlers usa a primeira ocorrência de cada propriedade (a genérica do site venceria mesmo com a da página presente). Corrigido removendo todas as tags `og:*`/`twitter:*` padrão antes de injetar as novas.
- **`e2e/07-finance-behaviors.spec.ts`** (regressão entre sessões, não um bug de produto): reaproveitava a conta QA compartilhada "Aluna QA Teste" pra criar vendas/recebimentos, e isso quebrou o teste de isolamento do Academy (`03-academy-and-isolation.spec.ts`, que espera `receivables` vazio via API direta pra uma aluna). Corrigido criando uma pessoa isolada por execução via `create_person()` (o RPC que a tela Pessoas usa — inserção direta na tabela `people` esbarra numa policy de RLS que não reproduzimos fora do fluxo real da aplicação, mesmo com os mesmos valores).
