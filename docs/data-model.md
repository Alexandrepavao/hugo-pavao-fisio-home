...
| 041 | dashboard_card_detail_expand | amplia `dashboard_card_detail` de 6 para 23 tipos de cartão (Início: attended/active_students/active_packages/active_partners/os 4 alertas restantes; Financeiro › Visão geral: average_ticket/attendance_rate/sales_confirmed/forecast_receivables_30d/forecast_payables_30d/cash_result; Financeiro › Recorrência: mrr_month/recurring_clients/churn_clients, mês de referência = mês de `p_from`, igual a `mrr_report`); corrige bug pré-existente nos 6 tipos originais onde `total_items` vinha de um `count(*)` sobre a subconsulta já limitada a 20 (subestimava o total real acima de 20) |
