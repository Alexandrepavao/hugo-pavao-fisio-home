// Templates de e-mail transacional do HP Group Hub — identidade HP, PT-BR, HTML responsivo + texto simples.
// Usado tanto pelo hook de autenticação do Supabase quanto pela função send-email da aplicação,
// para que todo e-mail (autenticação ou transacional) saia com a mesma marca.
...
// --- E-mails de autenticação (via Supabase Auth Send Email Hook) ---
...
// --- E-mails transacionais da aplicação (disparados pela própria aplicação, não pelo Supabase Auth) ---

export type AppEmailTemplate = "invite" | "appointment_confirmation" | "generic";

export function renderAppEmail(t: AppEmailTemplate, name: string, data: Record<string, string>): EmailContent {
  const n = name || "olá";
  if (t === "invite") {
    return renderAuthEmail("invite", data.link ?? "");
  }
  if (t === "appointment_confirmation") {
    const when = data.when ?? "";
    const unit = data.unit ? ` (${data.unit})` : "";
    return {
      subject: "Confirmação de atendimento — HP Fisioterapia",
      html: renderShell({
        preheader: "Seu atendimento está confirmado.",
        heading: "Atendimento confirmado",
        bodyHtml: `Olá, ${esc(n)}.<br><br>Seu atendimento está agendado para <strong>${esc(when)}</strong>${esc(unit)}.<br><br>Para remarcar, responda esta mensagem ou fale com nossa equipe.`,
      }),
      text: `Olá, ${n}.\n\nSeu atendimento está agendado para ${when}${unit}.\n\nPara remarcar, responda esta mensagem ou fale com nossa equipe.`,
    };
  }
  const subject = (data.subject ?? "Mensagem do HP Group").slice(0, 120);
  const bodyText = (data.body ?? "").slice(0, 4000);
  return {
    subject,
    html: renderShell({ preheader: subject, heading: subject, bodyHtml: esc(bodyText).replace(/\n/g, "<br>") }),
    text: bodyText,
  };
}
